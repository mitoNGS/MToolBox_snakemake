...
# changelog
# 16th May 2014
# - added "ref" attribute to Transition/Transversion classes. It should be added to Deletion too.
# - changed inheritance of SNP_MixIn class. It was "object" when defined but it could not be used alone to check if two SNP_MixIn instancies are equal to each other. Now the SNP_MixIn class inherits from BaseSNP directly. This has been tested as-it-is but it should be tested extensively.
...
# old definition of SNP_MixIn class
...
#        return False
#    def print_snp(self):
#        return super(SNP_MixIn, self).print_snp() + self.ref + ' -> ' + self.change
#    def print_table(self):
...
class Transition(SNP_MixIn, BaseSNP):
    def __init__(self, element=None, start=0, ref=None, change=''):
        #try:
        self.start=start
        if ref:
            # what to do if reference is provided
            self.ref=ref
        else:
            # ref is the nucleotide in RSRS
            self.ref=consts.RCRS[self.start-1]
        if element:
            #print "ELEMENT", element
            super(Transition, self).__init__(element)
            self.change = consts.TRS_TBL[self.ref]
        else:
            #print "NO ELEMENT"
...
class Transversion(SNP_MixIn, BaseSNP):
    def __init__(self, element=None, start=0, ref=None, change=''):
        ...
        try:
            ...
            print(self.start-1)
            pass
        if self.change not in consts.transversions[self.ref]:
            raise TypeError("Invalid Transversion (%s -> %s) provided!" % (self.ref,self.change))
