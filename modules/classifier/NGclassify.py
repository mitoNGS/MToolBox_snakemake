...
def get_snps_from_vcf_dict(vcf_dict, ref_seq=rsrs_seq):
    ...
    for snp in vcf_dict:
        if snp[-1] == 'del':
            ...
            ref = ref_seq[snp[0]-1]
            var = snp[1][0]
            if ((ref in consts.PUR and var in consts.PUR) or
                (ref in consts.PYR and var in consts.PYR)):
                mut = datatypes.Transition(snp[0])
            elif ((ref in consts.PUR and var in consts.PYR) or
                  (ref in consts.PYR and var in consts.PUR)):
        ...
        if x != y:
            if x != gap and y != gap:
                ...
                if ((x in consts.PUR and y in consts.PUR) or
                    ...
                    mutations.append(mut)
                # Trasversione
                elif ((x in consts.PUR and y in consts.PYR) or
                      (x in consts.PYR and y in consts.PUR)):
                ...
                # Ambiguity
                    ...
                    if y != 'N':
                        for i in consts.ambiguity[y]:
                            # retain mutation defined by ambiguity only if
                            ...
                            if i != x:
                                if ((x in consts.PUR and i in consts.PUR) or
                                    ...
                                    mut.ambiguity = y
                                    mutations.append(mut)
                                elif ((x in consts.PUR and i in consts.PYR) or
                                      (x in consts.PYR and i in consts.PUR)):
            ...
            # accrocchio per permettere di associare quelle riconosciute come
            # retromutazioni nel'albero con quelle che poi in rCRS sono presenti
...
def compare_mutations_regions(h_pos_list, s_pos_list, regions=None):
    ...
    if regions:
        for region in regions:
            ...
            h_pos_list_checked.extend(
                [x for x in h_pos_list
                 if x.start >= region_start and x.start <= region_end]
            )
    else:
    ...
    missing_haplo_pos = sorted([x for x in h_pos_list_checked
                                if x not in s_pos_list], key=lambda x: x.start)
    # check
...
#Alcune Sequenze non Vengono correttamente allineate da Muscle, sarebbe da
# implementare anche mafft?
...
class SequenceDiff(object):
    ...
    def gen_diff(self, muscle_exe=None, rif=None, obj=None):
        ...
        if rif:
            alg, self.rif, self.obj = self.gen_alg(muscle_exe=muscle_exe,
                                                   rif=rif, inc=obj)
            self.alg = alg
        ...
        if self.ignore_position:
            for i in self.ignore_position:
                if i in self.diff_list:
                    ...
                    print('_'*30)
                    print("**** Deleting SNP: {}".format(i.pprint()))
...
class Classify(object):
    ...
    def classify_haplogroup(self, haplo_name, haplo_list, seq_diff, regions):
        ...
        # check
        (matched, raw_tot, tot, missing_haplo_pos, h_pos_list_checked,
         s_pos_list) = compare_mutations_regions(haplo_list, seq_diff.diff_list,
                                                 regions=regions)
        if matched > 0:
...
#        self.seq_diff = seq_diff
    ...
    def classify90(self):
        ...
        self.haplo_stats90 is an OrderedDict (in decreasing order of P_Hg)
        version of self.haplo_stats keeping only predictions with P_Hg > 0.89
        """
        # test code, start
        # print dict((key, value) for key, value in self.haplo_stats.iteritems())
        # test code, end
        self.haplo_stats90 = dict((key, value) for key, value in self.haplo_stats.items()
    ...
    def pprint_sorted(self, outfile=sys.stdout):
        ...
        for haplo in self.haplo_stats_sorted.keys():
            ...
            missing_haplo_pos_tostring = ';'.join(
                [i.__str__() for i in self.missing_haplo_pos[haplo]]
            )
    ...
    def prediction_sorting(self):
        ...
        if s == "complete":
            ...
            self.haplo_stats_sorted = OrderedDict(
                sorted(self.haplo_stats.items(),
                       key=lambda x: (-x[1][0], -x[1][0]/float(x[1][1])))
            )
            # check if there are two haplogroups with max values of Nph and P_Hg
            # TODO: ma che cazzo
            self.haplo_best = OrderedDict(
                filter(lambda x: (x[1][0], x[1][0]/float(x[1][1])) == (self.haplo_stats_sorted.items()[0][1][0], self.haplo_stats_sorted.items()[0][1][0]/float(self.haplo_stats_sorted.items()[0][1][1])), self.haplo_stats_sorted.items()))
        else:
            t = "incomplete"
            ...
            if self.classify90():
                ...
                self.haplo_best = OrderedDict(
                    filter(lambda x: (x[1][0],x[1][0]/float(x[1][1])) == (best_matched,best_matched/float(best_tot)), zip(self.haplo_stats_sorted.keys(), self.haplo_stats_sorted.values())))
    ...
    def get_mhcss(self, mhcs_dict):
        ...
        self.mhcss = set([parse_mhcs.which_mhcs_lite(i, mhcs_dict)
                          for i in best_list])
        return self.mhcss
