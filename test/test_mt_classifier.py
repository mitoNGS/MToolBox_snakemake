...
def test_main_mt_hpred():
    """
    This test checks if the best prediction and the N, Nph_exp and Nph_tot are correct.
    """
    ...
    sc = main_mt_hpred(contig_file = contig_file, muscle_exe = muscle_exe, basename = basename, best_results_file = best_results_file, data_file = data_file)[0]
    assert list(sc.haplo_best.keys())[0] == "A1"
    assert list(sc.haplo_best.values())[0] == (45, 45, 45)    
